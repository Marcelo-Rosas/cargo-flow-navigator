...
| `/veiculos` | Veículos / Motoristas / Proprietários | autenticado |
...
| `/aprovacoes` | Aprovações | autenticado |
| `/gestao-usuarios` | Gestão de usuários | admin |
...
**Entrypoints:** `Commercial.tsx`, `QuoteCard.tsx`, `QuoteForm.tsx`, `QuoteFormWizard.tsx`, `QuoteDetailModal.tsx`, `useQuotes.ts`
...
| `buonny-check` | Integração Buonny/seguro |
...
- **Tipos:** `src/types/route-stops.ts` — `RouteStop`, `Waypoint`
