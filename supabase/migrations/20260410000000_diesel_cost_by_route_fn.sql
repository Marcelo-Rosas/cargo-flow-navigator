-- RPC: get_diesel_cost_by_route
-- Returns aggregated diesel cost per UF route pair, using historical ANP prices
-- matched to each CT-e issuance date. Consumption rate: 0.3 L/km (NTC standard).
...
AS $$
  WITH cte_with_diesel AS (
    ...
    LEFT JOIN LATERAL (
      ...
      ORDER BY pdp.periodo_coleta DESC
      LIMIT 1
    ) orig_p ON true
    LEFT JOIN LATERAL (
      SELECT pdp.preco_medio
