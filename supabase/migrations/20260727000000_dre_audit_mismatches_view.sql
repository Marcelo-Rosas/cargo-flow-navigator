-- View de auditoria DRE: identifica COT/OS com dados incompletos,
-- quebrando a regra contábil de comparativo Presumido vs Real.
...
WHERE q.stage = 'ganho'
  ...
  AND (
    (
      -- totals.das ou totals.icms ausentes ou zero
      COALESCE((q.pricing_breakdown->'totals'->>'das')::numeric, 0) = 0
      AND COALESCE((q.pricing_breakdown->'totals'->>'icms')::numeric, 0) = 0
    )
    OR
    (
      -- profitability.custoMotorista ou profitability.custosCarreteiro ausentes ou zero
