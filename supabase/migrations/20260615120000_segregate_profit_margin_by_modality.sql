...
-- ============================================================
INSERT INTO public.pricing_rules_config (key, label, category, value_type, value, metadata, is_active, vehicle_type_id)
SELECT
  ...
  'percentage',
  COALESCE(
    (SELECT value FROM public.pricing_rules_config WHERE key = 'profit_margin_percent' AND vehicle_type_id IS NULL LIMIT 1),
    15
  ),
  jsonb_build_object('description', 'Margem alvo de lucro para Lotação (FTL) — independente do Fracionado'),
  true,
  NULL
WHERE NOT EXISTS (
  SELECT 1 FROM public.pricing_rules_config WHERE key = 'profit_margin_lotacao_percent' AND vehicle_type_id IS NULL
);
...
SELECT
  ...
  jsonb_build_object('description', 'Margem alvo de lucro para Fracionado (LTL) — independente da Lotação'),
  true,
  NULL
WHERE NOT EXISTS (
  SELECT 1 FROM public.pricing_rules_config WHERE key = 'profit_margin_fracionado_percent' AND vehicle_type_id IS NULL
);
