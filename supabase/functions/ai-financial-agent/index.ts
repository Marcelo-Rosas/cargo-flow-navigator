...
import { executeQuoteProfitabilityWorker } from '../_shared/workers/quoteProfitabilityWorker.ts';
...
async function shouldSkipAi(sb: any, analysisType: string, entityId: string) {
  try {
    ...
    if (analysisType === 'quote_profitability') {
      ...
      const { data: quote } = await sb.from('quotes').select('value').eq('id', entityId).maybeSingle();
    ...
    if (analysisType === 'financial_anomaly') {
      ...
      const { data: doc } = await sb.from('financial_documents').select('total_amount').eq('id', entityId).maybeSingle();
...
// Cache Check
// ─────────────────────────────────────────────────────
async function getCachedInsight(sb: any, insightType: string, entityType: string | null, entityId: string | null) {
  try {
    let query = sb
      .from('ai_insights')
...
// ─────────────────────────────────────────────────────
// Context Enrichment — fetch previous insights for entity
// ─────────────────────────────────────────────────────
async function fetchPreviousInsights(sb: any, entityType: string | null, entityId: string | null): Promise<string | undefined> {
  ...
  try {
    ...
    if (!budget.allowed) {
      ...
      return jsonResponse({
        ...
        budget: { daily_pct: budget.daily_pct, monthly_pct: budget.monthly_pct, alert: budget.alert },
        fallback: {
          risk: 'medio',
    ...
    if (body.entityId && body.analysisType !== 'dashboard_insights') {
      ...
      if (skipCheck.skip) {
        await logUsage(sb, {
          analysisType: body.analysisType, model: 'none', costUsd: 0, status: 'cached',
          entityType: body.entityType, entityId: body.entityId, errorMessage: skipCheck.reason,
        });
        return jsonResponse({
          success: true, skipped: true, reason: skipCheck.reason,
          analysis: {
            risk: 'baixo',
    ...
    const cachedInsight = await getCachedInsight(
      sb, body.analysisType, body.entityType || null, body.entityId || null,
    );
    if (cachedInsight) {
      await logUsage(sb, {
        analysisType: body.analysisType, model: 'cache', costUsd: 0, status: 'cached',
        entityType: body.entityType, entityId: body.entityId,
      });
      return jsonResponse({ success: true, cached: true, analysis: cachedInsight }, 200, corsHeaders);
    }

    // 4. Select model
    const model = selectModel(body.analysisType);

    // 5. Fetch previous insights for context enrichment
    const previousInsights = await fetchPreviousInsights(
      sb, body.entityType || null, body.entityId || null,
    );

    // 6. Route to specialized worker
    let workerResult: any;
    switch (body.analysisType) {
      case 'quote_profitability':
        workerResult = await executeQuoteProfitabilityWorker({
          entityId: body.entityId, model, sb, previousInsights,
        });
        break;
      case 'financial_anomaly':
        workerResult = await executeFinancialAnomalyWorker({
          entityId: body.entityId, model, sb, previousInsights,
        });
        break;
      case 'approval_summary':
        workerResult = await executeApprovalSummaryWorker({
          entityId: body.entityId, entityType: body.entityType, model, sb, previousInsights,
        });
        break;
      case 'dashboard_insights':
        workerResult = await executeDashboardInsightsWorker({ model, sb });
        break;
      default:
        return jsonResponse({ error: `Unknown analysisType: ${body.analysisType}` }, 400, corsHeaders);
    }

    // 7. Log success
    await logUsage(sb, {
      analysisType: body.analysisType, model, costUsd: 0, status: 'success',
      entityType: body.entityType, entityId: body.entityId,
    ...
    await logUsage(sb, {
      analysisType: 'unknown', model: 'unknown', costUsd: 0, status: 'error',
      errorMessage: String(e),
    });
    return jsonResponse({ error: String(e), provider: 'llm' }, 500, corsHeaders);
  }
});
