import { createClient } from 'jsr:@supabase/supabase-js@2'; // P1.1 — fix import npm→jsr
import { getCorsHeaders } from '../_shared/cors.ts';
import {
  executeDriverQualificationWorker,
  type DriverQualificationWorkerResult,
} from '../_shared/workers/driverQualificationWorker.ts';
...
import {
  executeOperationalReportWorker,
  type OperationalReportData,
} from '../_shared/workers/operationalReportWorker.ts';
import {
  executeRegulatoryUpdateWorker,
  type ParsedArticle,
} from '../_shared/workers/regulatoryUpdateWorker.ts';
import {
  executeOperationalInsightsWorker,
  type OperationalInsightsData,
} from '../_shared/workers/operationalInsightsWorker.ts';
...
// Regulatory scraping helpers (moved from worker)
const SOURCE_URLS = [
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1. Budget check
    // P1.2 — stage_gate_validation never calls LLM → always skip budget
    // regulatory_update skips unless body.analyzeContent is explicitly set
    const needsLLM =
    ...
    // 4. Pre-fetch + call worker + post-persist
    ...
    switch (body.analysisType) {
      ...
      case 'stage_gate_validation': {
        ...
        workerResult = await executeStageGateWorker({ orderData, targetStage: body.targetStage });
      ...
      case 'compliance_check': {
        ...
        workerResult = await executeComplianceCheckWorker({
          orderData: order,
          quoteData: quote,
          checkType: (body.checkType || 'pre_coleta') as CheckType,
          model,
          previousInsights,
        });
      ...
      case 'regulatory_update': {
        ...
        if (articles.length === 0) {
          ...
          workerResult = await executeRegulatoryUpdateWorker({
      ...
      case 'operational_insights': {
        ...
        workerResult = await executeOperationalInsightsWorker({ operationalData, model });
