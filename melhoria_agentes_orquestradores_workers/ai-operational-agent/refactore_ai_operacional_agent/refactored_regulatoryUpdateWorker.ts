...
import {
  SYSTEM_PROMPT_REGULATORY,
  MAX_TOKENS_REGULATORY,
} from '../prompts/system_regulatory.ts';
...
interface RefactoredWorkerResult {
  ...
  durationMs: number;
  provider: string;
  notifications: Array<{ template: string; channel: string; payload: any; status: string; created_at: string }>;
...
export async function executeRegulatoryUpdateWorker(ctx: RefactoredWorkerContext): Promise<RefactoredWorkerResult> {
  const { articlesToProcess, model, previousInsights } = ctx;
  const startTime = Date.now();
  let provider = '';

  const regulatoryUpdatesData: Array<any> = [];
  const notifications: Array<{ template: string; channel: string; payload: any; status: string; created_at: string }> = [];
  ...
  return {
    analysisSummary: { new_articles: articlesToProcess.length, high_relevance_count: highRelevanceCount },
    durationMs,
    provider,
    notifications,
