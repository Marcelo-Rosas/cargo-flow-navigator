import { useMutation, useQueryClient } from '@tanstack/react-query';
import { supabase } from '@/integrations/supabase/client';
import { toast } from 'sonner';

interface ValidateDocumentResponse {
  status: 'pending' | 'valid' | 'invalid' | 'xml_parsed';
  document_type: 'nfe' | 'cte' | 'mdfe' | 'unknown';
  validation_errors: string[];
  metadata: {
    uf_code?: string;
    uf_name?: string;
    ano_mes?: string;
    cnpj_emitente?: string;
    modelo?: string;
    modelo_descricao?: string;
    serie?: string;
    numero?: string;
    digito_verificador?: string;
    dv_calculado?: string;
  };
  xml_data?: {
    chave?: string;
    emitente_cnpj?: string;
    emitente_nome?: string;
    destinatario_cnpj?: string;
    destinatario_nome?: string;
    valor_total?: string;
    data_emissao?: string;
    status?: string;
  };
}

export function useValidateDocument() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (documentId: string): Promise<ValidateDocumentResponse> => {
      const { data, error } = await supabase.functions.invoke('validate-document', {
        body: { documentId, auto_update: true },
      });

      if (error) throw error;
      return data as ValidateDocumentResponse;
    },
    onSuccess: (data, documentId) => {
      const statusLabel: Record<string, string> = {
        valid: 'Válido',
        pending: 'Pendente',
        invalid: 'Inválido',
        xml_parsed: 'XML Parseado',
      };

      if (data.status === 'valid' || data.status === 'xml_parsed') {
        toast.success(`Documento validado: ${statusLabel[data.status]}`, {
          description: data.metadata.modelo_descricao
            ? `${data.metadata.modelo_descricao} • ${data.metadata.uf_name} • Série ${data.metadata.serie} • Nº ${data.metadata.numero}`
            : undefined,
        });
      } else if (data.status === 'invalid') {
        toast.error('Documento inválido', {
          description: data.validation_errors?.[0] || 'Verifique a chave de acesso',
        });
      } else {
        toast.info(`Status: ${statusLabel[data.status] || data.status}`);
      }

      queryClient.invalidateQueries({ queryKey: ['documents'] });
    },
    onError: (error) => {
      toast.error('Erro na validação', {
        description: error instanceof Error ? error.message : 'Tente novamente',
      });
    },
  });
}
