...
$c = $c.Replace("    } catch {`n      // API pode estar indisponivel`n    }", "    } catch {`n      toast.error('Falha ao consultar CNPJ — verifique sua conexao');`n    }")
Set-Content $f $c -NoNewline -Encoding UTF8
...
Write-Host "`nPRONTO! Reinicie: npm run dev"
